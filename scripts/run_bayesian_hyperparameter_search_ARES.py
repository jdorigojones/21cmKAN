...
def train_model(config):
    ...
    val_dataset = NumPy2TensorDataset(features_npy_file=config["data_path"] + 'X_val_ARES.npy', 
                                    targets_npy_file=config["data_path"] + 'y_val_ARES.npy')    
    ...
    max_abs = torch.abs(y_val_ARES).max(dim=1)[0]

    # Grab training Dataset
    train_dataset = NumPy2TensorDataset(features_npy_file=config["data_path"] + 'X_train_ARES.npy', 
                                        targets_npy_file=config["data_path"] + 'y_train_ARES.npy')
    ...
    test_mean_rel_err, test_median_rel_err, test_max_rel_err = eval_test_set_ARES(model, data_path, device)
...
trial_directory = "test_experiment_ARES"
